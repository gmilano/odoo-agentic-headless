...
- [x] `AH-0103` Add GEAI/MCP tool adapter: `scripts/mcp-server.js` exposes all 11 Odoo agentic endpoints as MCP tools with typed schemas. GEAI agents connect via `mcpServers.odoo` config; profile injected per sector via `X-Agentic-Profile` header.
- [x] `AH-0104` Add permission profiles: executive read-only, ops operator, finance operator, admin.
- [x] `AH-0105` Add risk classifier for destructive or financial operations.
...
- 2026-06-25 08:01 America/Montevideo: completed `AH-0105` with a centralized risk classifier and authenticated `POST /agentic/v1/risk_classification`. A single `classify_operations_risk` now scores destructive (`unlink`/`delete`), arbitrary (`call`), financial (`account.move` and the accounting model family), and state-sensitive-field writes, returning overall risk, financial/destructive flags, `requires_approval`, and per-operation factor reasons; `infer_operations_risk` was refactored to delegate to it so approval-queue and execute_plan risk scoring share one policy. Endpoint added to capabilities `allowed_operations`, `risk_classifier` guardrail flag set, and stale next-safety-gaps/guardrail texts refreshed. Docker was down this run, so verified via py_compile plus a standalone classifier harness (read-only=low, create=medium, financial/destructive/state-change/call=high, mixed-plan=medium, backward-compat infer). Commit local; remote exists (origin GitHub) but not pushed per loop rule. Follow-up: wire per-operation risk factors into `action_plan` output and start `AH-0104` permission profiles.
- 2026-06-26 08:01 America/Montevideo: wired the AH-0105 risk classifier into `POST /agentic/v1/action_plan` output. Plans now ship a `risk_classification` block (risk, financial, destructive, requires_approval, requires_durable_approval_reference, summary) plus per-operation `risk_factors` from the centralized `classify_operations_risk`, replacing the heuristic `inferred_plan_risk`. While wiring it, found and fixed a real scoring gap: comprehension operations (`business_snapshot`, `business_events`, `business_cockpit`, `capabilities`) were "unrecognized" and pushed pure-review plans to medium/requires_approval; added them to `READ_ONLY_OPERATIONS` so review plans now score low and need no approval, while create/write plans stay medium. Removed the now-resolved action_plan next-safety-gap and bumped module to `19.0.1.13.0`. Docker was down this run, so verified via py_compile plus a standalone odoo-stub harness asserting review-plan=low/no-approval and create-plan=medium/approval. Commit `151a1c5` local; remote exists (origin GitHub) but not pushed per loop rule. Follow-up: start `AH-0104` permission profiles (executive/ops/finance/admin) and `AH-0103` Claude/Wany tool adapter.
- 2026-06-27 11:40 America/Montevideo: completed `AH-0103` as MCP server. `scripts/mcp-server.js` exposes all 11 Odoo agentic endpoints (business_snapshot, business_events, business_cockpit, action_plan, execute_plan, approval_requests, audit_logs, risk_classification, permission_profiles, capabilities, okf_bundle) as typed MCP tools via stdio JSON-RPC. Profile injected per-sector via `X-Agentic-Profile` header + `AGENTIC_HEADLESS_PROFILE` env. GEAI connects via its MCP Proxy config (`mcpServers.odoo`). Created companion project `company-as-code` with `company.yaml` schema, `examples/wany.yaml` (5 sectors: Research/Marketing/Finance/Operations/Leadership), and `scripts/setup.js` runner that bootstraps KB namespaces + GEAI assistants + GitHub repos from the manifest. Live run: created 5 GEAI assistants (ids saved in setup-report-wany.json), all routing through GEAI for full trace/audit. Follow-up: AH-0107 OKF materialization, Company as Code Cloudflare agentic readiness + Vercel+Auth0 dashboard.
- 2026-06-27 08:01 America/Montevideo: completed `AH-0104` permission profiles. Added a defense-in-depth role layer on top of the bearer token: `executive` (read/plan/audit/approval-request only), `ops` (non-financial create/write/execute_plan), `finance` (adds financial/accounting writes), and `admin` (full, incl. `call`; the default so existing callers are unaffected). The active profile is chosen per request via the `X-Agentic-Profile` header or globally via `AGENTIC_HEADLESS_PROFILE`; an unknown profile returns 403. `enforce_profile(operation, financial=...)` gates `create`/`write`/`execute_plan`/`call`, with financial scoping driven by `FINANCIAL_MODELS` and (for execute_plan) the centralized `classify_operations_risk`. New authenticated `GET|POST /agentic/v1/permission_profiles` lists profiles + active profile; capabilities now exposes `permission_profiles` block + guardrail flag and the resolved safety gap was removed. Bumped module to `19.0.1.14.0`. Docker was down this run, so verified via py_compile plus a standalone odoo-stub harness (27 checks: admin allows all, executive blocks all writes/call, ops blocks financial+call, finance allows financial but blocks call, env/header precedence, unknown-profile 403). Commit `2c0bbd7` local; remote exists (origin GitHub) but not pushed per loop rule. Follow-up: bind profiles to distinct API keys so callers cannot self-select a header, and implement `AH-0103` Claude/Wany tool adapter (which should send the right profile per tool surface).
